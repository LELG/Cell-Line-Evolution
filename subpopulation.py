...
class Subpopulation(object):
    ...
    """
    # a counter of how many subpopulations have been created.
    ...
    def __init__(self, opt, prolif, mut_rate, depth, t_curr, col, prev_time, inherited_mutns=None, from_file=False):
        ...
        self.__class__.num_clones_created += 1
        ...
        if from_file:
            # initialise attributes for reconstructing clone tree
            self.parent_id = None
            self.num_children = 0

    @classmethod
    def init_from_file(cls, opt, mutation_map, attr_dict):
        """Load a subpopulation from a single line of a population snapshot.

        Load a subpopulation from one line of a CSV population snapshot.
        We assume the line has been read in using a csv DictReader, and
        has the form
        ...
        First, we extract the attributes that are required for
        initialising the Subpopulation using the standard constructor,
        and the mutations dictionary, which is a special case.

        That having been done, we create the new clone with a
        call to Subpopulation(). We then iterate over the remaining
        attributes in attr_dict, updating new_clone's attributes
        accordingly.

        Finally, the mutations dictionary is reconstructed. The
        dictionary stored in the population file only contains IDs,
        rather than actual Mutation objects, so we need to go to
        the master list of mutations (indexed by ID in mutation_map)
        and retrieve the corresponding Mutations.

        Input
        -----
        cls: the class we are instantiating (most likely, Subpopulation)
        opt: the global parameter set, used to initialise the subpop
        mutation_map: a dictionary of all mutations in this simulation,
                      indexed by mutation ID so that we can re-populate
                      a list of Mutations from a list of IDs
        attr_dict: a dictionary of attributes generated by a DictReader
                   processing a stored population CSV file.

        Returns
        -------
        A new subpopulation, with attributes set from the dictionary.
        """
        # get attributes from dictionary, using literal_eval
        # to prevent malicious code being executed
        try:
            # get attributes we need to initialise Subpopulation
            prolif_rate = literal_eval(attr_dict.pop('prolif_rate'))
            mut_rate = literal_eval(attr_dict.pop('mut_rate'))
            depth = literal_eval(attr_dict.pop('depth'))

            # get mutations (should be a dictionary of lists)
            mutations = literal_eval(attr_dict.pop('mutations'))
        except KeyError:
            raise KeyError("attempting to load subpop from invalid attribute dict")

        # call Subpopulation() to create new clone
        new_clone = cls(opt=opt, prolif=prolif_rate, mut_rate=mut_rate,
                        depth=depth, t_curr=0, col='n', prev_time=0,
                        from_file=True)

        # Update attributes from file.
        # Only update attributes that the clone already has,
        # to prevent bad input from CSV file
        for attr in attr_dict:
            if hasattr(new_clone, attr):
                setattr(new_clone, attr, literal_eval(attr_dict[attr]))
            else:
                raise Exception("attempting to set invalid attribute for subpop: {}".format(attr))

        # reconstruct clone's mutations
        for mut_type in mutations:
            for mut_id in mutations[mut_type]:
                # get the Mutation corresponding to this id, if one exists
                mut = mutation_map.get(mut_id, None)
                if not mut:
                    raise Exception("trying to associate clone with non-existent mutation")
                # add the Mutation to the subpop's list of mutations
                try:
                    new_clone.mutations[mut_type].append(mut)
                except KeyError:
                    raise KeyError("Mutation has invalid type: {}".format(mut_type))
                # check whether this clone is the mutation's original clone
                try:
                    if new_clone.clone_id == mut.original_clone_id:
                        mut.original_clone = new_clone
                        del mut.original_clone_id
