...
mutation_values="0.0000001 0.000001 0.00001 0.0001 0.00025 0.0005"
prob_mut_values="0.01 0.1 0.25 0.5 0.75 0.9 0.99"
...
do
    for prob_mut_pos in $prob_mut_values
    do

        prob_mut_neg=$(echo $prob_mut_pos | awk '{ print 1-$1}')
        ...
        do
            ...
            python main.py -d $death_rate -p $proliferation_rate -m $mutation_rate --loops $loops --max_size_lim $max_size_lim --prolif_lim 0.0 --init_size $initial_size -f $filepath -s $selective_pressure -t $select_time --prob_mut_pos $prob_mut_pos --prob_mut_neg $prob_mut_neg --prob_inc_mut $prob_inc_mut --prob_dec_mut $prob_dec_mut --scale $scale --mscale $mscale --M -n $testname -g $testgroup --init_diversity $initial_diversity --sub_file $sub_file --Z --NP
